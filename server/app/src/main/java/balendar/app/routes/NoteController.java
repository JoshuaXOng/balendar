...
public class NoteController {
    ...
    public NoteController(MongoOperations mongoOps, CalendarNoteRepo noteRepository, ObjectMapper mapper) {
    ...
    public CalendarNoteDTO updateNote(@PathVariable String id, @RequestBody UpdateNoteDataDTO updateNoteData) {
        CalendarNote note = this.noteRepository.findOneById(id);
        if (note == null) 
            throw new ResourceNotFoundException();

        Map<String, Object> noNullNoteData = this.noNullMapper.convertValue(updateNoteData, Map.class);
        
        for (Field field : note.getClass().getDeclaredFields()) {
            if (noNullNoteData.containsKey(field.getName()))
                try { field.set(note, noNullNoteData.get(field.getName())); }
    ...
    public List<CalendarNoteDTO> deleteNotes() {
        ...
        List<CalendarNote> removedNotes = this.mongoOperations.findAllAndRemove(predicate, CalendarNote.class);
        
        return removedNotes.stream()
            .map(rn -> new CalendarNoteDTO(rn))
            .collect(Collectors.toList());
    }
}
